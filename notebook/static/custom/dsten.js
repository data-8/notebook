...
Customized for Data Science 8

Toggle on-off "Student Mode" using the check mark in the toolbar.

Student Mode contains the following modifications:

1. If a cell is executed out of order, then all above are executed as well
2. Only edit mode exists, and only for code cells:
	a. All command mode shortcuts are removed.
	b. The first code cell is selected upon page load.
	c. Markdown-formatted cells are not editable
3. "scratch cell" modal for testing code outside of the current environment.
...
define([
	...
	events.on('app_initialized.NotebookApp', function() {
		...
		- toggles a global student mode variable

		icon: fortawesome.github.io/Font-Awesome/icons

		*/

		STUDENT_CELL_CLASS = 'student-mode-cell';

		IPython.toolbar.add_buttons_group([{
			'label': 'Student Mode Toggle',
			'icon': 'fa-square-o',
			'callback': function() {
				var button = $(this).children('i');
				button.toggleClass('fa-square-o').toggleClass('fa-check-square-o');
				window.student = button.hasClass('fa-check-square-o');
				button.children('b').html(window.student ? 'on' : 'off');
				if (window.student) remove_command_mode();
		...
		function toggle(list, attr, option1, option2) {
			var option = window.student ? option1 : option2;
			for (var i in list) {
				item = list[i];
		...
		- add a label to the student-model button
		- select the first cell on load

		*/

		function initialize_student_mode() {
			$('.fa-square-o').html('	Student mode <b>on</b>').click()
				.parents('.btn-group').css('float', 'right');
			select_cell($('.cell:first-child'));
		}

		function pick_cell() {
			if ($('.'+STUDENT_CELL_CLASS).length == 0) {
				candidate = next_available_code_cell();
				candidate.addClass(STUDENT_CELL_CLASS);
			}
		}

		function unpick_cell() {
			$('.'+STUDENT_CELL_CLASS).remove();
		}
		...
		*/
		...
		function remove_command_mode() {
			...
			cmd.clear_shortcuts();
			edt.clear_shortcuts();
			add_student_shortcuts();
			$('.student_modal').show();
			$('#student_mode').attr('rel', 'stylesheet');
		}
		...
		function restore_command_mode() {
			...
			$('.student_modal').hide();
			$('#student_mode').attr('rel', 'student-mode-deactivated');
		}

		function add_student_shortcuts() {
		...
		*/
		...
		$(document).on('mouseenter', '.text_cell', function() {
			if (window.student) keep_state(select_cell, this);
		});
		...
		function next_available_cell(selector) {
			...
			if (is_empty(candidate)) {
				console.log('[Student Mode] Cell empty. Selecting cell.');
				return candidate;
			} else {
				console.log('[Student Mode] Cell not empty. Adding new cell.');
		...
		*/
		...
		function initialize_student_modal() {
			window.modal = false;
			$('#insert-cell-below').click();
		}

		window.toggle_student_modal = function() {
			if ($('.'+STUDENT_CELL_CLASS).length > 0) unpick_cell(); else pick_cell();
			$('.'+STUDENT_CELL_CLASS).toggleClass('shown');
			$('.student_modal').toggleClass('shown');
			window.modal = $('.'+STUDENT_CELL_CLASS).hasClass('shown');
			$('.student_modal .button').html(window.modal ? 'Deactivate' : 'Activate');
		}
		...
		$(document).ready(function() {
			...
			initialize_student_mode();
			initialize_student_modal();
			initialize_ok_tests();

			// DOM initializers

			$('head').append('<link href="/static/custom/dsten.css" rel="stylesheet" id="student_mode">');
			...
			$('#notebook').append('<div class="student_modal"><div class="student_text"><h3>Scratch Cell</h3>' + '<p>"Scratch" offers a small sandbox environment, independent of your IPython notebook. ' + 'Shift+Enter with the Scratch Cell open to run it.</div><div class="button" ' + 'onclick="toggle_student_modal()">Activate</div></div>');
		});
	});
});
